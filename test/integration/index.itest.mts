 ...
 * real admin to the dev database. Every document carries an `itest-…@marketplace.invalid`
 ...
 * `additionalProperties: false` and owns the field shape, so writing through it directly keeps
 ...
 * checkUserAuthorizationDisDel cases below reduce to exactly this and differ only in how the admin
...
describe('refresh-cookie gate over HTTP', () => {
	...
	// the _id it points at matches no admin. This is the one case that exercises both
	...
	// The other MongoDB branch tokenInfoAdmin can take once it DOES find the document: a real
	// admin, accepted by the real validator, that checkUserAuthorizationDisDel must reject. The unit
	// suite only proves the handler propagates whatever that gate throws — it mocks the gate itself
	// — so this is the one test that proves the real marketplace-common gate actually rejects a
	// `disabled: true` document read back out of the real database, not a stub standing in for it.
	it('answers 401 when the live session points at an admin MongoDB has marked disabled', async () => {
		const { _id } = await seedAdmin({ disabled: true })
		// Read the seed back through the raw driver before the request runs: without this, a 401
		// here would be equally consistent with the insert having silently failed (tokenInfoAdmin's
		...
		expect(await db().collection('admin').findOne({ _id })).toMatchObject({ disabled: true })
	...
	it('answers 401 when the live session points at an admin MongoDB has marked deleted', async () => {
		const { _id } = await seedAdmin({ deleted: new Date() })
		...
		const seeded = await db().collection('admin').findOne({ _id })
...
describe('refresh rotates the session on the cluster', () => {
	...
	it('writes the new pair, arms both TTLs, and deletes the refresh token it consumed', async () => {
		...
		await redisClient.hSet(oldRefreshKey, { _id: _id.toHexString(), tier: TIER.admin, ...lineage })
		...
		expect(await redisClient.hGetAll(newRefreshKey)).toEqual({ _id: _id.toHexString(), tier: TIER.admin, ...lineage })
