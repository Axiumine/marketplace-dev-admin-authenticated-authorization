...
`custom.yml` here carries `marketplace-no-log-auth-token`, the rule common to
every service. This service is the Admin tier's token-lifecycle authority
(`refresh` mutation, Redis session read/write, signed-cookie verification — see
CLAUDE.md), so it applies directly. It does **not** carry `marketplace-no-log-reset-secret`: that rule
