 ...
 * attempt limiter, which runs on every call, and it is the grace counter, which runs only on a replay
 * inside the window. So the steady-state tests pin the key it was called with rather than that it was
 * never called: the limiter must have counted once, and nothing else may have.
 *
 * It once counted a third thing — `dual-read-hits`, the fallback that let a pre-cutover session
...
/*
 * The lineage every refresh hash carries, and which `assertRefreshLineage` refuses a session without:
 * a family the rotations of one login share, the instant that login happened, and the number of days
 * it may go on rotating for. The clock is frozen so `originalLogin` can be a literal — a session
 * stamped `Date.now()` at fixture-build time would age between the fixture and the assertion.
 */
...
describe('adminAuthenticatedAuthorizationHandler', () => {
	...
	it('files the replay on the account trail, with no token anywhere in the line', async () => {
		...
		// Fifty entries, thirty days: the two bounds the reuse trail keeps, arriving on the same append.
	...
	/*
	 * The introspection bypass, which until the auth-boundary contract was tested only in the four services
	 * that were not an authorization one. The bypass is a development convenience and outside `development`
	 * and `test` it does not exist: the gate is read before the code is, so the configured value is never
	 * consulted and the header is worth exactly what a header nobody sent is worth.
	 */
	describe('outside the environment allowlist', () => {
		...
		const refusal = async (header: Record<string, string>) => {
			try {
				await adminAuthenticatedAuthorizationHandler(keys)(makeCtx(header), next)
			} catch (error) {
	...
	it('refuses the twenty-first attempt of a minute without reading anything', async () => {
		...
		await expect(adminAuthenticatedAuthorizationHandler(keys)(makeCtx({ cookie: signedCookie() }), next)).rejects.toMatchObject({
