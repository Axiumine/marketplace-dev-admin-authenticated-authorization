...
//     that at test/integration/*.itest.mts would hammer the real Redis cluster AND
//     the real MongoDB hundreds of times per mutant, inside this service's own
//     `marketplaceDev:itest:adminAuthenticatedAuthorization:` namespace, where
//     `fileParallelism: false` serialises everything. The namespace being per-service
//     rather than shared does not make that cheaper — it is still real round trips
//     to real infrastructure hundreds of times over. Unit tests mock both
