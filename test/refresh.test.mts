...
/*
 * Where a rotated session is written: the shared prefix plus the SHA-256 of the **prefixed** token.
 * `access:` and `refresh:` live inside the digest — they are what tells the two hashes apart — and the
 * digests are written out as literals, computed elsewhere: hashing the tokens here with the call the
 * code makes would agree with it about any algorithm, including a mutated one.
 */
...
// just retired still usable. One shape only: there is no raw-token key beside it.
 ...
 * predecessor's is removed — and both field names are the bodies of the session keys above, because the
 * revocation built on it rebuilds `${REDIS_KEY}${field}` and never sees a token.
 */
...
describe('refresh mutation', () => {
	...
	it('rotates both tokens, re-seeds Redis, sets the cookie and drops the used refresh', async () => {
		...
		// a cluster — and there is no third, because no raw-token shape exists beside them.
