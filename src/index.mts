...
import { adminAuthenticatedAuthorizationHandler } from '@lib/auth/adminAuthenticatedAuthorizationHandler.mjs'
...
export const ENDPOINT = '/admin-authenticated-authorization'
...
export function logListening(env: NodeJS.ProcessEnv = process.env): void {
	...
	// there is no single address to name. HOSTNAME used to appear here, but it never had any
	// effect on what was actually bound.
...
export async function createServer(keygripKeys: IKeygripKeyMaterial[]) {
	...
	app.use(async (ctx: IContextAuthenticatedAuthorization, next: Next) => {
		...
		await adminAuthenticatedAuthorizationHandler(app.keys as Keygrip)(ctx, next)
	})
