# Semgrep — marketplace-dev-admin-authenticated-authorization
...
`custom.yml` here carries the two rules common to every service —
`marketplace-no-log-introspection-code` and `marketplace-no-log-auth-token`. This
service is the Admin tier's token-lifecycle authority (`refresh` mutation,
Redis session read/write, signed-cookie verification — see CLAUDE.md), so both
apply directly. It does **not** carry `marketplace-no-log-reset-secret`: that rule
only makes sense on `marketplace-dev-public-resource`, the one tier reachable
without authentication and the only place the password-reset flow (and its
`resetHash`/`resetDateReq` fields) exists. This service has no reset-password
material at all (verified: `grep -ri resetpwd\|resethash\|resetdatereq src`
finds nothing), so the rule would just be dead weight here.
...
- **Scope is the explicit file list, not `.semgrepignore`.** Because files are
  ...
  Note that `src/index.ts.orig` is a merge leftover and is correctly out of
  scope — the `find` list has no `.orig` entry.
- **Parsed-lines ≈ 64.3%, not 100%, and that is expected.** The bypass parses each
